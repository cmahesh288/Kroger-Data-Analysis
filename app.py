...
def Search():
    ...
    if request.method == 'POST' and 'search' in request.form :
        ...
        if not re.match(r'\d+', number):
            ...
            conn = get_conn()
            cur = conn.cursor()
            cur.execute("SELECT h.HSHD_NUM, t.BASKET_NUM, t.PURCHASE_DATE, p.PRODUCT_NUM, p.DEPARTMENT, p.COMMODITY,t.SPEND, t.UNITS, t.STORE_R, t.WEEK_NUM, t.YEAR, h.L, h.AGE_RANGE,h.MARITAL,h.INCOME_RANGE, h.HOMEOWNER, h.HSHD_COMPOSITION, h.HH_SIZE, h.CHILDREN FROM 400_households AS h JOIN 400_transactions AS t ON h.HSHD_NUM = t.HSHD_NUM JOIN 400_products AS p ON t.PRODUCT_NUM = p.PRODUCT_NUM where h.HSHD_NUM=? ORDER BY h.HSHD_NUM, CAST(t.BASKET_NUM AS UNSIGNED),STR_TO_DATE(t.PURCHASE_DATE, '%d/%b/%y'), CAST(p.PRODUCT_NUM AS UNSIGNED), p.DEPARTMENT, p.COMMODITY" ,number)
            data=cur.fetchall()
            if data:
        ...
        cur = conn.cursor()
        testquery="SELECT h.HSHD_NUM, t.BASKET_NUM, t.PURCHASE_DATE, p.PRODUCT_NUM, p.DEPARTMENT, p.COMMODITY,t.SPEND, t.UNITS, t.STORE_R, t.WEEK_NUM, t.YEAR, h.L, h.AGE_RANGE,h.MARITAL,h.INCOME_RANGE, h.HOMEOWNER, h.HSHD_COMPOSITION, h.HH_SIZE, h.CHILDREN FROM 400_households AS h JOIN 400_transactions AS t ON h.HSHD_NUM = t.HSHD_NUM JOIN 400_products AS p ON t.PRODUCT_NUM = p.PRODUCT_NUM WHERE h.HSHD_NUM=10 ORDER BY h.HSHD_NUM, CAST(t.BASKET_NUM AS UNSIGNED), STR_TO_DATE(t.PURCHASE_DATE, '%d/%b/%y'), CAST(p.PRODUCT_NUM AS UNSIGNED), p.DEPARTMENT, p.COMMODITY"
